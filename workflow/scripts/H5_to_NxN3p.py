...
def H5_to_NxN3p(matrix: str, chrom: str, start: int, end: int):
    hic = hm.hiCMatrix(matrix)
    matrix = hic.matrix.toarray()

    if start is not None:
        idx1, idx2 = hic.getRegionBinRange(hic.getChrNames()[0], start, end)
        matrix = matrix[idx1:idx2, idx1:idx2]
...
def parseArgs():
    ...
    parser.add_argument(
        '--chrom', help='Chrom of simulation.')
    parser.add_argument(
        '--start', type=int, help='Start coordinate of simulation.')
    parser.add_argument(
        '--end', type=int, help='End coordinate of simulation.')
