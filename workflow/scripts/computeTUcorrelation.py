...
def main(file: str, **kwargs) -> None:
    ...
    allDistances = pd.read_csv(file)
    allDistances = allDistances.pivot(
        index='timepoint', columns='TU', values='activated')

    # Compute Pearson correlation coefficient and convert to long format
    correlations = allDistances.corr(method='pearson').stack()
    ...
    pvalues = allDistances.corr(method=pearsonr_pval).stack()
...
def parse_arguments():
    ...
    custom.add_argument(
        'file',
        help='TU activation states - output of computeTUactivation.py')
